...
resolve_codex_cli() {
  ...
  shopt -s nullglob
  for glob_path in "$HOME"/.antigravity/extensions/openai.chatgpt-*-darwin-arm64/bin/macos-aarch64/codex; do
...
OPENCODE_USE_GLOBAL_XDG="${AUDITHOUND_OPENCODE_USE_GLOBAL_XDG:-1}"
...
case "$AGENT_TYPE" in
  ...
  opencode)
    ...
    if [ "$OPENCODE_USE_GLOBAL_XDG" = "1" ]; then
      "$OPENCODE_BIN" run \
        --dir "$TARGET_DIR" \
      ...
      XDG_ROOT="$AGENT_DIR/.opencode_xdg"
      mkdir -p "$XDG_ROOT/data" "$XDG_ROOT/cache" "$XDG_ROOT/state" "$XDG_ROOT/config"

      # Preserve the user's configured providers/models while keeping
      # per-worker state/cache/databases isolated.
      if [ -d "$HOME/.config/opencode" ] && [ ! -e "$XDG_ROOT/config/opencode/opencode.jsonc" ]; then
        mkdir -p "$XDG_ROOT/config"
        cp -R "$HOME/.config/opencode" "$XDG_ROOT/config/"
      fi
      if [ -f "$HOME/.local/share/opencode/auth.json" ] && [ ! -e "$XDG_ROOT/data/opencode_auth.json" ]; then
        cp "$HOME/.local/share/opencode/auth.json" "$XDG_ROOT/data/opencode_auth.json"
      fi
      if [ -f "$HOME/.local/state/opencode/model.json" ] && [ ! -e "$XDG_ROOT/state/opencode/model.json" ]; then
        mkdir -p "$XDG_ROOT/state/opencode"
        cp "$HOME/.local/state/opencode/model.json" "$XDG_ROOT/state/opencode/model.json"
      fi
