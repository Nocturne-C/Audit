...
def run_opencode_summary(prompt: str, target_dir: Path, model: str, round_dir: Path) -> tuple[str, str]:
    opencode_bin = resolve_opencode_cli()
    task_file = round_dir / "round_summary_task.md"
...
def main() -> int:
    ...
    retained = [item for item in findings if isinstance(item, dict) and item.get("round") == args.round_num]
    ...
    (round_dir / "round_summary.md").write_text(stdout.strip() + "\n", encoding="utf-8")
    print(json.dumps({"path": str(round_dir / "round_summary.md")}, ensure_ascii=True))
    return 0
